...
var create = function ( points, callback ) {
    ...
    // callback that detect when all frames files are finished
    var filesWritedCallback = function (err) {
        ...
        if (err) {
            ...
            finishedFilesCount++;
            if (finishedFilesCount === points.length - 1 ) {
                callback();
            }
        }
    ...
    for ( var i = 1; i < points.length; i ++ ) {
        ...
        fs.writeFile( __dirname + '/image-simple/frame_' + ( "00000" + i ).slice( -5 ) + '.png', base64Data, 'base64', function ( err ) {

            if ( err ) filesWritedCallback( err );
            filesWritedCallback();
